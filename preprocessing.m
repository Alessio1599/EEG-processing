%% Load the data; transform from single to double precision
...
%% Possible Resampling (here, resampling is not applied)
...
%% Plot the signals over the entire duration 
...
%% Compute and plot the power spectrum density of all signals and also plot the PSD of electrodes n.10,11,53
...
%% Filtering with FIR filter (Kaiser window) COMMENTED 
...
%% Filtering (low-pass, high-pass, notch) with IIR filters (elliptic) 
...
%% Plot the filtered signals over the entire duration
...
%% Compute the power spectrum density of all filtered signals and compare the PSD before vs after filtering (electr. 10,11,53)
...
%% Plot the markers of the events over the n filtered signals 
...
%% Extract epochs [-0.2 0.8]s around each stimulus presentation--> 3-D matrix
...
%% Concatenation of extracted epochs 3-D matrix --> 2-D matrix
...
%% Identification and plot of bad channels 
...
%% Saving at the end of these pre-processing steps
